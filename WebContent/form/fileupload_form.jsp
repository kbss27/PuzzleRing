...
<body>
	<%
		...
		try {
			...
			//확장자
			String extension = uploadFile.substring(uploadFile
					.lastIndexOf(".") + 1);
			...
			if (!(extension.equalsIgnoreCase("java"))) {
				...
				DB_Id = id;
				DB_projectName = request.getParameter("project_name");
				DB_className = uploadFile.substring(0,
				...
				fileName = DB_className + "_" + DB_Id + "_"
						+ simDf.format(new Date(currentTime)) + "-" 
						+ dd.format(new Date(currentTime)) + "."
						+ extension;
				System.out.println("저장된 이름 : " + fileName);

				upload = new PRUpload();
				//filename, id, date, projectname, classname
				upload.upLoad(fileName, DB_Id, DB_date, DB_projectName,
						DB_className);
